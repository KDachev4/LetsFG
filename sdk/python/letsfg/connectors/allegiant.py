...
class AllegiantConnectorClient:
    ...
    async def search_flights(self, req: FlightSearchRequest) -> FlightSearchResponse:
        ...
        if ob_result.offers:
            ...
            try:
                ...
                logger.debug("G4: ancillary fetch timed out")
            except Exception as _anc_err:
                logger.debug("G4: ancillary fetch error: %s", _anc_err)
        return ob_result
    ...
    async def _fetch_ancillaries(
        ...
        cache_key = f"g4_{origin}_{dest}_{date_str}"
        now = time.time()
        ...
        for o in ob[:15]:
            for i in ib[:10]:
                ...
                combos.append(FlightOffer(
                    id=f"rt_alle_{cid}", price=price, currency=o.currency,
