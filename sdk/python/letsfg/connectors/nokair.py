...
class NokAirConnectorClient:
    ...
    async def _fetch_ancillaries(
        ...
        """Return Nok Air (DD) ancillary pricing from published tariff reference."""
        from .ancillary_ref import get_ancillary_ref
        ref = get_ancillary_ref("DD")
        if not ref:
            return None
        ...
        for o in ob[:15]:
            for i in ib[:10]:
                ...
                combos.append(FlightOffer(
                    id=f"rt_noka_{cid}", price=price, currency=o.currency,
