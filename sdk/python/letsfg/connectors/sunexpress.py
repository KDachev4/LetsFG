...
class SunExpressConnectorClient:
        ...
        if ob_offers:
            ...
            try:
                ancillary = await asyncio.wait_for(
                    self._fetch_ancillaries(anc_origin, anc_dest, req.date_from.isoformat(), req.adults, ob_offers[0].currency or req.currency or "EUR"),
                    timeout=45.0,
                )
                if ancillary:
    ...
    async def _fetch_ancillaries(
        ...
        # SunExpress XQ — Essential fare: cabin bag 8 kg free, checked bag add-on
        return {
            "checked_bag_note": "not included (Essential fare – cabin bag 8 kg free)",
            "bags_note": "checked bag 20 kg add-on from ~10 EUR",
